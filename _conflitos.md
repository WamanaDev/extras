...
## 47. Next.js atualizado de 15.0.3 → 15.5.25 — vulnerabilidade de segurança sinalizada pela própria Vercel no deploy

Vercel avisou no log de deploy: "Vulnerable version of Next.js detected, please update immediately." `next@15.0.3` é afetado por CVE-2025-29927 (bypass de autorização via header `x-middleware-subrequest` em `middleware.ts` — permite pular a checagem de auth/CSRF do middleware inteiro), corrigido a partir de `15.2.3`. Especialmente crítico aqui porque `src/middleware.ts` é onde `security-headers.ts` roda globalmente e onde a spec espera que nenhuma rota escape das checagens do pipeline (`contrato-comum.md`).

**Corrigido**: `next` e `eslint-config-next` (que deve acompanhar a versão do framework) atualizados pra `15.5.25` — última versão estável da mesma major (15.x), evitando o salto pra major 16 (menor risco de quebra em App Router/Server Components no meio do projeto). `@playwright/test` também precisou subir (`1.48.0` → `1.63.0`) porque `next@15.5.25` declara peer dependency `^1.51.1` nele. `tsc --noEmit`, lint completo e suíte de testes (664/665, a única falha é o flake de timing de sempre, confirmado em isolamento) continuam limpos após o bump — nenhuma mudança de código foi necessária além das versões em `package.json`.

## 48. Login quebrado em produção: CSP bloqueava os próprios scripts inline do Next.js — faltava o header também na requisição

Usuário reportou (console do navegador, domínio próprio já com SSL funcionando): "Executing inline script violates the following Content Security Policy directive 'script-src... nonce-...'" repetidas vezes na página de login, seguido de "Connection closed" — a página carregava mas o JS de hidratação nunca rodava.

Causa: o Next.js App Router injeta seus PRÓPRIOS `<script>` inline (payload de hidratação, streaming de Server Components) e só sabe aplicar `nonce="..."` automaticamente neles se o header `Content-Security-Policy` (com o nonce) estiver presente nos headers da **requisição** que o middleware repassa adiante — não basta estar na resposta. `src/middleware.ts` só setava `Content-Security-Policy` em `response.headers` (via `aplicarHeadersSeguranca`); a requisição só levava um header próprio `x-csp-nonce`, que o Next não lê pra esse propósito. Sem o CSP no request, os scripts que o próprio framework injeta saem sem nonce — o navegador bloqueia todos eles, e a aplicação nunca hidrata (daí "Connection closed" logo em seguida: o React nunca assume o DOM).

Não aparecia em nenhum teste porque `security-headers.test.ts` testa só `aplicarHeadersSeguranca` isoladamente (a função que seta a RESPOSTA) — nunca havia teste de `middleware.ts` em si, então a falta do header na requisição passou despercebida até rodar de verdade num navegador.

**Corrigido**: `contentSecurityPolicy` (antes privada em `security-headers.ts`) foi exportada; `middleware.ts` agora calcula o valor da CSP uma vez e seta o MESMO valor tanto em `headers` (a cópia que vai pro `NextResponse.next({ request: { headers } })`, repassada ao Next) quanto em `response.headers` — mesmo nonce nos dois lugares, exatamente o padrão documentado pelo próprio Next.js pra CSP com nonce em App Router. Teste novo `src/middleware.test.ts` (nenhum existia antes) confirma que o nonce do CSP da resposta bate com o nonce do CSP repassado à requisição (via `x-middleware-request-content-security-policy`, mecanismo interno do Next pra expor headers de requisição sobrescritos), e que outros headers do request original (`X-Requested-With`, usado pelo CSRF) continuam intactos.

## 48b. Complemento — faltava `'strict-dynamic'` no `script-src`, item 48 não resolvia sozinho

Mesmo depois do item 48 (CSP presente tanto na requisição quanto na resposta, mesmo nonce), o erro persistiu idêntico em produção — mesmos hashes SHA-256 bloqueados. Faltava `'strict-dynamic'` em `script-src`: é parte obrigatória da receita oficial do Next.js pra CSP com nonce em App Router (nextjs.org/docs, "Content Security Policy"), não opcional. Sem ele, só o primeiro `<script>` que carrega com o nonce é confiável — os demais scripts que o PRÓPRIO Next injeta em runtime (payload de hidratação por Suspense boundary, streaming de Server Components, cada um com hash de conteúdo diferente a cada build) continuam bloqueados mesmo com nonce correto em request+response, porque eles não recebem o atributo `nonce` diretamente.

`'strict-dynamic'` resolve isso propagando a confiança do script com nonce pros scripts que ELE injeta dinamicamente — é assim que o navegador aceita os scripts internos do framework sem precisar listar um hash por script. Navegador que não suporta `strict-dynamic` (CSP Level 2) ignora a palavra-chave e cai em `'self'`, que continua listado como fallback.

`contentSecurityPolicy` (`security-headers.ts`) atualizada: `script-src 'self' 'nonce-...' 'strict-dynamic'`. Teste novo garantindo a presença de `'strict-dynamic'` junto do nonce em `script-src`.

## 48c. Complemento — faltava forçar renderização dinâmica; sem isso o nonce da CSP nunca batia (bloqueava até scripts com `src=`)

Mesmo com o nonce presente em request+response (item 48) e `'strict-dynamic'` no `script-src` (item 48b), o erro persistia — agora incluindo `<script src="...">` bloqueados ("host-based allowlisting is disabled" por causa do `strict-dynamic`), além dos inline de sempre. Causa raiz definitiva: `/login` (como praticamente toda rota client-only sem `headers()`/`cookies()` em algum Server Component da árvore) é PRÉ-RENDERIZADA em build time pelo Next — o HTML gerado então, com os `<script>` que o próprio framework injeta, fica fixo, sem nenhuma relação com o nonce que o middleware gera a cada requisição real. Resultado: nonce do CSP (na resposta) e nonce embutido no HTML (do build) NUNCA batem, pra nenhuma rota estática — nem "strict-dynamic" resolve isso, porque o problema não é a política em si, é a ausência de nonce válido pra começo de conversa nos scripts do HTML servido.

**Corrigido**: `src/app/layout.tsx` (raiz, envolve toda a árvore) virou `async` e chama `await headers()` — só isso já força o Next a renderizar TODA a aplicação dinamicamente, por requisição, e nesse caminho o framework embute o nonce real (o mesmo que está no header `Content-Security-Policy` daquela requisição) nos próprios `<script>` que injeta. É o trade-off documentado oficialmente pra CSP com nonce em App Router — não dá pra ter nonce por request E página estática ao mesmo tempo. Como o app já é majoritariamente autenticado/dinâmico (sessão, cookies, dados por usuário em quase toda tela), o custo de desativar a otimização estática global é aceitável aqui.

Sintoma colateral que o usuário reportou separado ("apareceu a tela mas o botão não funciona"): não é bug à parte — é o MESMO problema. Com os scripts bloqueados, o React nunca hidrata a página; o HTML aparece (foi servido normalmente), mas nenhum `onClick`/`onChange`/`onSubmit` está de fato ligado até a hidratação rodar. Corrigido o nonce, a hidratação volta a rodar e a interatividade volta sozinha.

Teste novo `src/app/layout.test.tsx` garante que `headers()` continua sendo chamado (única forma de travar essa regressão específica, já que ela não aparece em nenhum teste de página isolada — só em produção servindo HTML real).

## 49. Painel admin com 403 SEM_PERMISSAO após login correto — `resolverSessaoPadrao` sempre testava colaborador primeiro, incondicionalmente

Usuário logou como admin e todas as rotas `ator: 'ADMIN'` (`/api/admin/ciclos`, `/api/admin/colaboradores`) voltavam `403 SEM_PERMISSAO` — confirmado no corpo da resposta, não CSRF nem outra coisa. Causa: `resolverSessaoPadrao` (resolvedor de sessão padrão de `defineHandler`) sempre tentava resolver `sessao_colaborador` (cookie) PRIMEIRO, incondicionalmente, e só caía pra Supabase (admin) se o cookie de colaborador estivesse ausente — nunca olhava pra que tipo de ator a ROTA exigia. Quem tinha logado como colaborador em algum momento no mesmo navegador (comum em teste manual, ou em qualquer sessão que reaproveite o navegador entre os dois papéis) e depois logava como admin numa aba/sessão nova acabava com os dois cookies válidos ao mesmo tempo — a rota `ator: 'ADMIN'` recebia de volta a sessão de COLABORADOR (resolvida primeiro) e nunca chegava a checar o cookie de admin, caindo sempre em `403 SEM_PERMISSAO` mesmo com o login de admin certo.

**Corrigido**: `ResolverSessao` (tipo) ganhou um 3º parâmetro, `atorEsperado: TipoAtorRota` — `defineHandler` agora repassa `config.ator` ao chamar o resolvedor (`deps.resolverSessao(request, ip, config.ator)`). `resolverSessaoPadrao` usa isso pra tentar primeiro o tipo que a rota exige (`ADMIN` → checa Supabase primeiro, cai pro cookie de colaborador só se não achar; `COLABORADOR` → ordem inversa; `QUALQUER`/`PUBLICO` mantém a ordem histórica). Testes novos: `handler.test.ts` (pipeline repassa `config.ator` corretamente) e `resolver-sessao-padrao.test.ts` (novo arquivo — mocka Prisma e Supabase pra provar os 4 cenários de prioridade com os dois cookies presentes/ausentes em combinação).

## 50. Push automático de lembrete de extra confirmada — primeiro gatilho de negócio real pra infra de push (pedido do usuário)

Pedido: "notificações push sempre 4 horas antes de um dia de extra confirmado... todo dia às 08:00 quem tem extra no dia noturno e 20:00 quem tem extra no dia seguinte diurno." Confirmado em conversa: os horários reais dos turnos (diurno 07:00–19:00, noturno 19:00–07:00) dão ~11h de antecedência com 08:00/20:00, não 4h ao pé da letra — "4 horas antes" foi só a explicação aproximada do motivo; os dois horários fixos são a regra que vale.

`src/server/notificacoes/push.ts`/`criar.ts` já existiam como "preparação de infra" desde antes desta sessão, sem nenhum gatilho de negócio real — este é o primeiro.

**Implementado**:
- `src/server/notificacoes/lembrete-extra.ts` (novo) — `enviarLembretesDeExtra(prisma, turno, agora)`: `NOTURNO` aponta pro plantão de HOJE, `DIURNO` pro de AMANHÃ (regra fixa, não calculada a partir de "4 horas"). Consulta `marcacao` com `status: 'CONFIRMADA'` + `plantao.data`/`plantao.tipo`, chama `criarNotificacao` por colaborador. Dedupe por `link` (`/minhas-extras?marcacaoId=<id>`) contra notificação já existente pro mesmo colaborador — evita duplicar se o cron disparar mais de uma vez no mesmo dia (best-effort, sem constraint de banco).
- `src/app/api/cron/lembrete-extra/route.ts` (novo) — `GET ?turno=DIURNO|NOTURNO`. Foge do pipeline de `defineHandler` de propósito: é um webhook interno da plataforma (Vercel Cron), não uma rota chamada por sessão de usuário — nenhum dos 4 tipos de `ator` do contrato comum modela isso. Autenticado por `CRON_SECRET` (`Authorization: Bearer <segredo>`) — variável nova em `src/env.ts` (opcional, mesmo padrão de VAPID: sem ela a rota só recusa tudo, não quebra o boot) e `.env.example`.
- `vercel.json` (novo, projeto não tinha nenhum) — dois crons: `?turno=NOTURNO` às `0 11 * * *` (08:00 BRT) e `?turno=DIURNO` às `0 23 * * *` (20:00 BRT); `America/Sao_Paulo` é UTC-3 fixo, sem horário de verão desde 2019 (mesma constante já usada em `lib/escala/blocos.ts`).

Testes novos: `lembrete-extra.test.ts` (7 casos — qual dia cada turno consulta, mensagem com horário/RT corretos, dedupe, múltiplos colaboradores) e `route.test.ts` do cron (6 casos — auth por segredo, validação de `turno`, 500 nunca vaza detalhe técnico).

## 51. Notificação automática a todos os colaboradores ativos ao publicar um ciclo (pedido do usuário)

`publicarCiclo` (`src/server/ciclos/publicar.ts`, API-ADM-CIC-005) agora notifica TODOS os colaboradores `ativo: true` (não só os com escala no ciclo — pedido literal do usuário) quando a publicação é bem-sucedida, via `criarNotificacao` (tipo `CICLO_PUBLICADO`, mensagem com a competência do ciclo — "A escala de Setembro/2026 foi publicada.", link `/minha-escala`).

Disparado SÓ depois do `emTransacao` retornar (mesmo padrão de `broadcastEscalaAtualizada` em `escala/lote/route.ts`) — nunca antes do commit. Envolto em try/catch próprio (diferente de `criarNotificacao`/`enviarPushParaColaborador`, que só protegem o *push*): sem isso, uma falha ao GRAVAR uma notificação faria a chamada inteira rejeitar DEPOIS que o ciclo já tinha sido publicado de verdade — o admin veria erro na tela pra uma publicação que na verdade deu certo. Falha aqui só loga (`redigirParaLog`), nunca desfaz nem reporta erro da publicação.

Testes novos em `publicar.test.ts`: notifica cada colaborador ativo com o texto certo, falha ao notificar não derruba a publicação já commitada, nenhum colaborador ativo não chama `criarNotificacao`.

## 52. "Minha escala" (colaborador) mostrava "Extras: 0" com extra confirmada de verdade — mesma causa raiz do item 39, lado colaborador

Usuário relatou: colaborador #32285 com uma extra confirmada, mas o resumo da página `/minha-escala` mostrava "Extras: 0". Causa idêntica ao item 39 (grade admin), só que num código totalmente separado: `buscarMinhaEscala` (`API-COL-002`) buscava a extra confirmada via `LEFT JOIN LATERAL` ancorado em `escala_dia e` — se o dia da extra não tem linha de `escala_dia` (comum: a extra cai justo no dia de folga da pessoa), o dia INTEIRO ficava fora do `SELECT ... FROM escala_dia e ...`, extra incluída, e nem chegava a aparecer no resultado pra ser contado.

**Corrigido**: extras agora vêm de uma consulta SEPARADA (`marcacao` + `plantao` + `rt`, filtrada só por `colaboradorId`/`status=CONFIRMADA`/`ciclo_id` — nunca ancorada em `escala_dia`), casada por data com os dias que TÊM `escala_dia` só pra exibição inline (campo `dia.extra`); os TOTAIS (`totais.extras`, horas de extra) somam direto dessa consulta independente, nunca de `dias`. Mesmo padrão de correção do item 39 (fonte de extras desacoplada da existência de célula/linha de escala). Teste novo reproduz exatamente o cenário relatado: zero linhas de `escala_dia`, uma extra confirmada — `totais.extras` agora sai `1`, não `0`.

## 53. Integração com Google Calendar (OAuth2) — pedido do usuário, primeira integração externa de verdade do projeto

Pedido: botão pro colaborador conectar a conta Google e marcar automaticamente os dias trabalhados do ciclo + extras confirmadas na agenda pessoal. Usuário forneceu Client ID/Client Secret reais de um app OAuth criado no Google Cloud Console (contas de colaborador são Gmail pessoal, não Workspace — app fica em modo "Externo"; usuário foi avisado que precisa cadastrar usuários de teste ou enviar pra verificação do Google antes de liberar geral, já que `calendar.events` é escopo sensível).

**Sem dependência nova** (`googleapis` é um SDK pesado pra só 3 chamadas HTTP) — `fetch` direto contra os endpoints REST do Google (`oauth2.googleapis.com/token`, `www.googleapis.com/calendar/v3`), mesmo espírito de `push.ts`.

- **Schema**: `GoogleCalendarConta` (nova) — `colaboradorId` único, `refreshTokenCifrado` (AES-256-GCM via `CALENDAR_TOKEN_KEY`, nunca em claro — precisa ser REVERSÍVEL, diferente de hash de PIN/senha, porque a app usa o token de verdade pra chamar a API do Google), `calendarioId` (default `"primary"`). Migration `20260905130000_google_calendar_conta`, aplicada via `prisma migrate deploy` (mesmo motivo de sempre — shadow db do `migrate dev` falha contra publication do Supabase).
- **Env novas** (`src/env.ts`, todas opcionais — sem elas a integração fica desabilitada, nunca quebra o boot): `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` (Google Cloud Console), `CALENDAR_TOKEN_KEY` (32 bytes hex, `openssl rand -hex 32` — gerada e salva em `.env.local` nesta sessão).
- **`src/server/integracoes/google-calendar.ts`** (novo): cifra/decifra do refresh token, monta a URL de autorização (`access_type=offline&prompt=consent` sempre juntos — sem isso o Google só devolve `refresh_token` na primeira autorização de cada pessoa), troca `code`→tokens, renova `access_token` a partir do `refresh_token`, e `upsertEvento` (idempotente: tenta `POST` criar, se `409` já existe faz `PATCH`). Ids de evento determinísticos a partir do id da linha no banco (`idEventoEscala`/`idEventoExtra`, UUID sem hífens — já é um subconjunto válido do charset que o Google exige, `^[a-v0-9]{5,1024}$`) — sincronizar de novo nunca duplica.
- **Rotas**: `GET /api/colaborador/google-calendar/conectar` (inicia OAuth, redirect) e `GET .../callback` (troca code por tokens, salva) fogem do pipeline de `defineHandler` de propósito — mesmo motivo do cron (`item 50`): resposta é sempre um redirect de página inteira, nunca um envelope JSON; usam `resolverSessaoPadrao` diretamente pra exigir sessão de colaborador. `state` num cookie `httpOnly` de 10min protege contra CSRF do fluxo OAuth (RFC 6749 §10.12). `POST .../sincronizar` (marca o ciclo pedido) e `GET`/`DELETE /api/colaborador/google-calendar` (status/desconectar) já são JSON normal, via `defineHandler`.
- **Regra de negócio** (`google-calendar-sincronizar.ts`): só dias com `codigoEscala.presenca = true` (dias TRABALHADOS) viram evento — folga não tem utilidade numa agenda pessoal. Extras confirmadas sempre viram evento, independente de `escala_dia` (mesma filosofia dos itens 39/52 — mas aqui nem precisa de cuidado especial, já que a consulta de extras é uma tabela separada desde o início).
- **UI**: `<GoogleCalendarBotao />` em `/minha-escala` — "Conectar" é uma navegação de página inteira (`<a href>`, não `fetch`, porque OAuth exige redirect de documento completo), "Sincronizar"/"Desconectar" são chamadas normais. Precisa de `<Suspense>` ao redor (usa `useSearchParams` pra ler `?googleCalendar=conectado|erro|recusado` do retorno do OAuth).

Testes novos: `google-calendar.test.ts` (14 casos — cifra/decifra reversível com IV diferente a cada chamada, URL de autorização com os parâmetros certos, troca de code por tokens, renovação de access token, upsert create-ou-update de evento) e `google-calendar-sincronizar.test.ts` (5 casos — sem conta lança erro claro, um evento por dia trabalhado + um por extra, folga nunca vira evento, idempotência).

## 54. RN-16 estendida: extra NOTURNA que "termina" num dia com folga/férias também vira EM_AUSENCIA — LIMITE RÍGIDO (corpo de `marcar_extra`, FN-005)

**Autorização explícita do usuário nesta conversa** para alterar o corpo de `marcar_extra` — mesma exceção já usada quando FN-004/FN-005 foram implementadas pela primeira vez (`20260101000007_funcoes`). Usuário descreveu o cenário exato: colaborador NOTURNO, par (âncora 02/09/2026), tem 20/09 convertido de `'D'` pra folga (override pontual via `API-ADM-ESC-002`, não a rotação regular — dias de folga da rotação normal nunca materializam `escala_dia`). O pedido: uma extra NOTURNA em 19/09 (19:00 → 20/09 07:00) não pode ser marcada, porque ela "termina" dentro da madrugada do dia de folga; o mesmo vale pra qualquer extra (diurna ou noturna) no próprio dia 20/09 (esse caso já era coberto); e a mesma regra vale pra férias (é só outro código de ausência, o passo 7 antigo já era agnóstico ao código específico).

**Causa raiz do buraco**: o passo 7 de `marcar_extra` ("ausência no dia") só olhava `escala_dia.data = plantao.data` — o dia NOMINAL do plantão. Como `escala_dia` do dia de folga (20/09) guarda o intervalo `20/09 19:00 → 21/09 07:00` (mesma `hora_inicio`/`hora_fim` que a linha tinha quando ainda era `'D'` — `API-ADM-ESC-002` só troca `codigo_escala_id`, nunca as horas), o intervalo da folga e o intervalo do plantão-extra de 19/09 (`19/09 19:00 → 20/09 07:00`) não se sobrepõem literalmente — `valida_descanso` (FN-004) não acusa nada, e o passo 7 antigo também não, porque compara datas exatas, não intervalos.

**Corrigido** (`prisma/migrations/20260906130000_fn005_fn007_ausencia_turno_seguinte/migration.sql`, `CREATE OR REPLACE FUNCTION`, aplicada no banco real via `prisma migrate deploy`):
- Passo 7 de `marcar_extra` agora verifica `escala_dia.data IN (plantao.data, CASE WHEN plantao.tipo = 'NOTURNO' THEN plantao.data + 1 END)` — quando o turno é DIURNO a segunda posição é `NULL` e nunca casa com nada (comportamento idêntico ao de antes); quando é NOTURNO, o dia seguinte também entra na varredura. Bloqueia se qualquer um dos dois tiver ausência (`codigo <> 'D'`) e `permite_extra_em_folga` estiver desligado.
- **`plantoes_para_colaborador` (FN-007) mudou junto, na mesma migration** — não está na lista de limite rígido, mas seu `motivo` precisa continuar batendo exatamente com o que `marcar_extra` lançaria pro mesmo par (plantão, colaborador), nota que já existia no próprio corpo de FN-007 antes desta mudança ("os dois pontos de decisão convergem... não reimplementa a regra"). Sem esse ajuste espelhado, a grade de extras do colaborador mostraria o plantão de 19/09 como disponível, e o clique falharia com `EM_AUSENCIA` — UX inconsistente, e quebra do próprio invariante que FN-007 documenta.
- Verificado contra o banco real (Supabase), não só typecheck/lint: script ad-hoc rodou os três cenários dentro de transações com `ROLLBACK` (nenhum dado de teste ficou no banco) — NOTURNO em 19/09 lança `EM_AUSENCIA`; DIURNO em 19/09 (mesma véspera, não cruza meia-noite) passa e incrementa `vagas_ocupadas`; `plantoes_para_colaborador` relata `motivo: 'EM_AUSENCIA'` pro mesmo plantão NOTURNO.

**Achado colateral, não corrigido (fora do escopo deste pedido)**: os fixtures pgTAP de `fn-005-marcar-extra.pgtap.sql`/`fn-007-plantoes-para-colaborador.pgtap.sql` inserem `colaborador` com colunas `cpf_hash`/`cpf_ultimos4`, removidas em `20260105000001_remove_cpf_colaborador` — essas suítes já estavam desatualizadas em relação ao schema atual, independente desta mudança (nunca foram executadas neste ambiente, sem Docker — "PENDENTE DE EXECUÇÃO" no cabeçalho de ambos os arquivos). Os novos testes acrescentados aqui (F5-15/F5-16, F7-8c) seguem o mesmo padrão de fixture do resto do arquivo (portanto herdam esse mesmo problema de execução), mas foram verificados de verdade contra o banco real via script ad-hoc (ver item acima) — a regra em si está confirmada correta, só a suíte pgTAP como um todo precisa de uma passada de atualização de schema antes de rodar (não feita aqui, para não expandir o escopo do pedido original).

Testes novos: `fn-005-marcar-extra.pgtap.sql` F5-15 (NOTURNO na véspera de F → `EM_AUSENCIA`) e F5-16 (controle negativo: DIURNO na véspera de F → permitido); `fn-007-plantoes-para-colaborador.pgtap.sql` F7-8c (mesmo cenário, convergência de motivo entre as duas funções, mesmo padrão de F7-8a/b).

## 55. `/admin/ciclos/:id/marcacoes` — opção de criar um plantão novo (1 vaga, exclusivo) e já marcar nele, num único passo (pedido do usuário)

Pedido: além de atribuir uma extra manualmente num plantão já existente, poder criar um plantão novo só para aquela marcação (ex.: nenhum plantão daquele dia/turno/RT tem vaga sobrando, ou o admin quer uma vaga dedicada a uma pessoa específica sem afetar o pool geral).

**Decisão de atomicidade**: criar o plantão e marcar a extra são DUAS operações de escrita — juntá-las com duas chamadas HTTP sequenciais do cliente violaria a regra de `specs/AGENTS.md` ("nenhuma mutação de dados nasce fora de transação; nunca compor estado com duas chamadas"), porque um plantão criado com sucesso mas cuja marcação falhasse depois (`EM_AUSENCIA`, `EXCEDE_JORNADA`, `CRUZADA_BLOQUEADA`, etc.) ficaria órfão no banco — vaga fantasma, nunca preenchida, sem nenhuma marcação nela. Resolvido dentro da MESMA transação: `marcarExtraAdmin` (`src/server/services/marcacoes-admin.ts`, API-ADM-MAR-002) agora aceita `novoPlantao` como alternativa a `plantaoId` — quando presente, chama `criarPlantao` (`@/server/plantoes/criar`, API-ADM-PLA-001, reaproveitado sem duplicar suas regras de `CICLO_FECHADO`/`DATA_FORA_DO_CICLO`/`PLANTAO_JA_EXISTE`) com `vagasTotais: 1` fixo, ANTES de chamar `marcar_extra` (FN-005) — tudo dentro do `emTransacao` que já existia. Se `marcar_extra` rejeitar depois, o `ROLLBACK` desfaz a criação do plantão junto: nunca sobra uma vaga fantasma.

- **Schema da rota** (`POST /api/admin/marcacoes`): `plantaoId` virou opcional, novo campo `novoPlantao: { cicloId, rtId, data, tipo, horaInicio?, horaFim?, permiteCruzada }` opcional — `.refine()` exige exatamente um dos dois (nem os dois, nem nenhum).
- **Auditoria**: payload de `EXTRA_MARCADA` ganhou `plantaoCriadoParaEstaMarcacao: boolean`, pra distinguir no histórico se a marcação usou um plantão pré-existente ou um criado na hora — sem isso a auditoria não deixaria claro que uma vaga nova nasceu ali.
- **UI** (`/admin/ciclos/:id/marcacoes`): checkbox "Criar um plantão novo (1 vaga, exclusivo) só para esta marcação" — quando marcado, troca o select de RT (que normalmente só lista RTs que JÁ têm plantão naquele dia/turno, vindo de `plantoesDoDiaTurno`) por um select de TODAS as RTs (`/api/admin/rts`, mesmo endpoint já usado em `/admin/colaboradores`), já que o plantão ainda não existe. Botão muda de "Marcar" para "Criar plantão e marcar".

Testes novos em `marcacoes-admin.test.ts` (4 casos): cria o plantão com `vagasTotais: 1` antes de marcar_extra usando o id recém-criado; erro de `marcar_extra` depois de criar o plantão propaga intacto (mesma transação — quem garante o rollback de verdade é `emTransacao`/Prisma `$transaction`, já coberto em `db/tx.test.ts`, não este mock); erro de `criarPlantao` (`CICLO_FECHADO`) propaga sem nunca chamar `marcar_extra`; auditoria marca `plantaoCriadoParaEstaMarcacao: true`.

## 56. Colaborador não cancela mais a própria extra direto — pedido de cancelamento com aprovação de QUALQUER admin (pedido do usuário)

Pedido literal: "ao clicar em cancelar é aberto um pedido para um administrador confirmar o cancelamento... não precisa ser um administrador específico, qualquer administrador poderá confirmar ou recusar."

**Antes**: `DELETE /api/marcacoes/:id` (API-COL-005) chamava `cancelar_extra` (FN-006) direto com origem `COLABORADOR` — cancelava a marcação na hora. **Agora**: a mesma URL/verbo (deliberado — evita reescrever os 3 lugares da UI que já chamam `del('/api/marcacoes/:id')`, embora só um deles — `/minhas-extras` — realmente tivesse um botão funcional; `GradePlantoes.tsx` tinha uma função `cancelar` morta, nunca chamada, então `/plantoes` e `/plantoes-calendario` não precisaram de nenhuma mudança) só ABRE um pedido (`solicitacao_cancelamento`, `PENDENTE`). O cancelamento de fato só acontece quando um admin — QUALQUER admin, sem checagem de qual — aprova, reaproveitando `cancelar_extra` (FN-006) com origem `ADMIN`, exatamente como `cancelarExtraAdmin` já fazia (nenhuma lógica de cancelamento nova).

**Schema** (`SolicitacaoCancelamento`, migration `20260907120000_solicitacao_cancelamento`, aplicada via `prisma migrate deploy`): `marcacaoId`, `colaboradorId`, `motivo` (do colaborador, obrigatório), `status` (`PENDENTE`/`APROVADA`/`RECUSADA`), `motivoResolucao` (do admin — obrigatório numa recusa, opcional numa aprovação), `resolvidoPorId` (uuid de `auth.users`, sem FK — mesmo padrão de `troca_escala.criadoPorId`; "qualquer admin" é por isso que não há tabela local de admins para referenciar). Índice único PARCIAL (`WHERE status = 'PENDENTE'`) garante nunca mais de um pedido pendente por marcação — escrito à mão na migration (Prisma não modela `@@unique` com `WHERE` no schema); decisão final é do banco, o serviço só faz um `findFirst` de conveniência antes (idempotência de UI), e trata a violação de unicidade (23505) como "devolve o que já existe" em vez de propagar 500 numa corrida real.

- **`src/server/services/colaborador/solicitar-cancelamento.ts`** (novo, substitui `cancelar-extra.ts`/`cancelar-extra.test.ts`, removidos — ficaram inteiramente obsoletos): valida marcação própria (404 uniforme pra de-terceiro/inexistente, `SEC-CONF`), `CONFIRMADA`, ciclo não `FECHADO`; idempotente (pedido `PENDENTE` já existente é devolvido, não duplicado).
- **`src/server/services/solicitacoes-cancelamento.ts`** (novo): `listarSolicitacoesCancelamento` (admin, paginada), `aprovarSolicitacaoCancelamento` (chama `cancelar_extra` origem ADMIN dentro da transação, marca `APROVADA`) e `recusarSolicitacaoCancelamento` (nunca toca a marcação, só marca `RECUSADA` com `motivoResolucao` obrigatório). As duas notificam o colaborador (`criarNotificacao`) DEPOIS do commit, em try/catch próprio — mesmo padrão de `publicarCiclo` (item 51): falha ao notificar nunca desfaz nem reporta erro de uma resolução que já aconteceu de verdade.
- **Rotas novas**: `GET /api/admin/solicitacoes-cancelamento` (listar, paginado), `POST .../:id/aprovar`, `POST .../:id/recusar` — sem spec de API própria ainda, mesmo padrão aditivo já usado nesta sessão para `google-calendar`/`push`/notificações manuais.
- **`API-COL-006` (`buscarMinhasMarcacoes`)**: `podeCancelar` agora também considera se já existe um pedido `PENDENTE` (não pode abrir dois); novo campo `cancelamentoPendente` pra UI mostrar "Cancelamento em análise" em vez do botão.
- **UI colaborador** (`/minhas-extras`): modal trocado de `<ConfirmacaoImpacto>` (checkbox "li e entendo") por um `<Dialog>` com campo de motivo obrigatório — é o que o admin vê antes de decidir. Item com pedido pendente mostra um badge, nunca o botão.
- **UI admin** (`/admin/solicitacoes-cancelamento`, novo, link adicionado em `AdminNav`): fila filtrável por status (padrão: Pendentes), Aprovar de 1 clique, Recusar abre modal pedindo o motivo (obrigatório).
- **`AcaoAuditoria`**: `CANCELAMENTO_SOLICITADO`/`CANCELAMENTO_APROVADO`/`CANCELAMENTO_RECUSADO`, extensão aditiva (mesmo padrão de `NOTIFICACAO_ENVIADA`/`CODIGO_ESCALA_*`).

**Verificado contra o banco real** (Supabase, transações com `ROLLBACK`, nenhum dado ficou): índice único parcial rejeita um segundo `PENDENTE` pra mesma marcação (`23505`); aprovar cancela a marcação de verdade e decrementa `vagas_ocupadas`; recusar nunca toca marcação nem vagas.

Testes novos: `solicitar-cancelamento.test.ts` (7 casos), `solicitacoes-cancelamento.test.ts` (10 casos — inclui "qualquer admin", nenhuma checagem de dono do pedido), `minhas-marcacoes.test.ts` (+3 casos de `cancelamentoPendente`), `_MinhasExtrasClient.test.tsx` (reescrito pro novo fluxo de motivo obrigatório).

## 57. Navegação de mês em `/plantoes-calendario` e `/minha-escala-calendario` — chevrons viram funcionais, só ciclos PUBLICADO (pedido do usuário)

Pedido: os chevrons de `/plantoes-calendario` (decorativos até então — `<div>` com dois ícones, sem `onClick`) precisavam navegar entre meses; `/minha-escala-calendario` precisava do mesmo, do zero (não tinha chevron nenhum). Regra explícita do usuário: a navegação só enxerga ciclos `PUBLICADO` — "ciclos fechados são ciclos cancelados, não servem pra nada, nem para estatísticas" — e deve sempre manter carregado o anterior/atual/próximo, buscando +1 a cada passo pra nunca mostrar tela de carregando ao trocar de mês.

**Novo endpoint** `GET /api/ciclos/vizinhos?ano=&mes=` (`@/server/services/colaborador/ciclos-vizinhos.ts`) — busca todos os ciclos `PUBLICADO` (tabela pequena, ~1/mês, resolvido em memória) e devolve `{ anterior, atual, proximo }` por ordenação `(ano,mes)`; `RASCUNHO`/`FECHADO` nunca entram no cálculo (filtrados na própria query, nem chegam a ser candidatos a vizinho) — confirmado contra o banco real: com set/2026 e out/2026 `PUBLICADO` e nov/2026 `RASCUNHO`, pedir vizinhos de out/2026 devolve `proximo: null` (não pula pro rascunho de novembro). `atual` pode ser `null` (mês sem ciclo publicado) sem travar a navegação — `anterior`/`proximo` continuam resolvendo.

**`src/hooks/useNavegacaoCiclos.ts`** (novo, compartilhado pelas duas páginas): mantém a janela dos 3 ciclos; ao navegar, troca `atual` IMEDIATAMENTE a partir do vizinho já em memória (sem esperar rede) e só then busca `/api/ciclos/vizinhos` de novo centrado no novo `atual`, revelando o próximo passo (+1) em segundo plano.

- **`_CalendarioPlantoesClient.tsx`**: prop trocou de `cicloId/ano/mes` fixos pra `cicloInicial: CicloResumo`; dados de `/api/plantoes` agora cacheados por `cicloId` (`Record<string, GradePlantoesDados>`) — a navegação dispara prefetch dos vizinhos assim que a janela os revela, então o clique no chevron é instantâneo na maioria das vezes (só pede rede se o usuário navegar mais rápido que o prefetch). `usePlantoesRealtime`/`SaldoExtras` já reagiam a mudança de `cicloId` via prop, sem precisar de ajuste.
- **`_CalendarioEscalaClient.tsx`**: mesmo padrão — trocou `ano/mes` fixos por `cicloInicial`, e ganhou os chevrons que não existiam (mesmo visual/comportamento de `_CalendarioPlantoesClient`). Dados de `/api/minha-escala` + `/api/minhas-marcacoes` cacheados juntos por `cicloId` (`DadosCiclo { escala, marcacoes? }`).
- `GoogleCalendarBotao` (ambas as páginas) permanece atrelado ao ciclo do carregamento inicial da página (SSR) — não acompanha a navegação client-side; fora do pedido do usuário, não ajustado aqui.

Testes novos: `ciclos-vizinhos.test.ts` (8 casos — exato/sem-match, pula FECHADO/RASCUNHO, virada de ano, extremos da lista, janela calculada contra `agora`), `useNavegacaoCiclos.test.ts` (5 casos — busca ao montar, troca síncrona antes da rede, completa a janela +1, sem vizinho não faz nada), `_CalendarioPlantoesClient.test.tsx` e `_CalendarioEscalaClient.test.tsx` (4 casos cada — nenhum dos dois tinha teste antes).
